...
int find_section_by_name_and_type(Elf32_Shdr *section_tab2, Elf32_Shdr *section_tab, char *section_name_table, int num_sections, char *section_name)
{
    ...
    {
        if (section_tab[i].sh_type == section_tab2[i].sh_type && strcmp(section_name_table + section_tab2[i].sh_name, section_name) == 0) {
            return i;
        }
    }
    return -1;
...
void afficheTableSections(FILE *f, Elf32_Ehdr ehdr, Elf32_Shdr *section,FILE *f2, Elf32_Ehdr ehdr2, Elf32_Shdr *section2)
{
    ...
    {int l=0;
        ...
        if(find_section_by_name_and_type(section2, section, section_name_table, ehdr.e_shnum,tmp + section[i].sh_name )!=-1){
            l=find_section_by_name_and_type(section2, section, section_name_table, ehdr.e_shnum,tmp + section[i].sh_name );
