...
# verification header
function ver_header()
{	./readelf $1 -h > resultat_projet_p.txt
	readelf --wide -h $1 > resultat_commande_p.txt
	...
	diff -b -B resultat_projet.txt resultat_commande.txt 
	if [[ $? -eq 0 ]]
	then
...
{	./readelf $1 -S > resultat_projet.txt
	readelf --wide -S $1 > resultat_commande.txt
...
{	./readelf $1 -r > resultat_projet_p.txt
	readelf --wide -r $1 > resultat_commande_p.txt
	...
	diff -b -B resultat_projet_p3.txt resultat_commande_p4.txt 
	if [[ $? -eq 0 ]]
	then
